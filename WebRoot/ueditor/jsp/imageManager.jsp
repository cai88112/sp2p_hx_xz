...
<%@ page import="com.yfapp.platform.util.ConfigUtils"%>
<% 
    //仅做示例用，请自行修改
	String path = "upload";
	String imgStr ="";
	String realpath = "";
	String tempRoot = getRealPath(request,path);
	if(tempRoot.endsWith("/")){
		realpath = getRealPath(request,path)+path;		
	}else{
		realpath = getRealPath(request,path)+File.separator+path;
	}
	System.out.println("real.display:"+realpath);
...
public String getRealPath(HttpServletRequest request,String path){
	...
	String rootDir = ConfigUtils.getProperty("file.path");
	if(null == rootDir || "".equals(rootDir.trim())){
