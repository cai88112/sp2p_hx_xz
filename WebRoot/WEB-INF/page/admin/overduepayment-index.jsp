						...
						<tbody>
						    <tr>
								<td class="f66" align="left" width="50%" height="36px">
									...
									借款类型：
									<s:select id="borrowWay" list="#{-1:'--请选择--',1:'净值借款',3:'信用借款',4:'实地考察借款',5:'机构担保借款'}"></s:select>
								    &nbsp;&nbsp;
									<s:select id="timeType" list="#{-1:'--请选择--',1:'垫付时间',2:'应还时间'}"></s:select>
								    &nbsp;&nbsp;
...
<script type="text/javascript">
			$(function(){
				...
				$("#excel").attr("disabled",true);
				  window.location.href=encodeURI(encodeURI("exportlateRepay.do?status=1"+"&&userName="+$("#userName").val()+"&&status="+$("#status").val()+"&&borrowWay="+$("#borrowWay").val()+"&&timeType="+$("#timeType").val()+"&&timeStart="+$("#timeStart").val()+"&&timeEnd="+$("#timeEnd").val()));
			...
			function initListInfo(praData){
				...
				praData["paramMap.timeType"] = $("#timeType").val();
			...
			function repaySubmit(obj,repaySum){
			    var r=confirm("是否确定还款?");
	            if(r == true){
	               param['paramMap.id'] = obj;
	               param['paramMap.repaySum'] = repaySum;
