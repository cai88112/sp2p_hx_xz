...
<body>
	...
	<div class="nymain">
		<div class="ifbox1">
			<h2>
				<span><a href="javascript:void(0);" id="sendmail">发送站内信</a> <a
					href="javascript:void(0);" id="reportuser">举报此人</a></span>标的详情
			</h2>
			<div class="boxmain">
				<div class="pic-info">
					<div class="tx">
						<shove:img src="${borrowDetailMap.imgPath}"
							defaulImg="images/default-img.jpg" width="128px;" height="128px;">
						</shove:img>
					</div>
					<div class="guanzhu">
						<!-- Baidu Button BEGIN -->
	...
	document.getElementById("bdshell_js").src = "http://bdimg.share.baidu.com/static/js/shell_v2.js?cdnversion="
				...
				<div class="xqbox">
					...
					<div class="xqboxmain">
						<div class="xqtop">
							<div class="money">
								<p>
									借款金额：<strong>￥<s:property
											value="#request.borrowDetailMap.borrowAmount" default="---" /></strong>
								</p>
								<p>
									借款年利率：<span style="margin-right: 15px;"><s:property
											value="#request.borrowDetailMap.annualRate" default="---" />%(月利率：<s:property
											value="#request.borrowDetailMap.monthRate" default="---" />%)</span>
									借款期限：<span><s:property
											value="#request.borrowDetailMap.deadline" default="---" /> <s:if
											test="%{#request.borrowDetailMap.isDayThe ==1}">个月</s:if>
								...
								<s:if test="#session.DEMO==2">
									...
									<s:elseif test="%{#request.borrowDetailMap.borrowStatus == 2}">
										<a href="javascript:void(0);" id="invest"><img
											src="images/neiye2_07.jpg" width="124" height="50" /></a>
									</s:elseif>
						...
						<div class="xqbottom">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
									<td width="27%">还差：<span>￥ <s:property
												value="#request.borrowDetailMap.investAmount" default="0" /></span>
									</td>
									<td width="37%">
										<div>
									...
									<td width="20%"><s:if test="#request.subscribes!=1">&nbsp;</s:if>
										<s:else>
    		剩余份数:<s:property
								...
								<tr>
									...
									<td><strong style="color: red"> 还款方式： <!-- modify by houli 如果是天标，则还款方式为  到期还本付息 -->
    	 ...
    	 到期还本付息
									...
									<td>投标奖励：<s:if
											test="%{#request.borrowDetailMap.excitationType == 1}">无</s:if>
										<s:elseif
	    ...
	    ￥<s:property value="#request.borrowDetailMap.maxTenderedSum"
									...
									<s:else>
										<td>最小认购金额： ￥<s:property
												value="#request.borrowDetailMap.smallestFlowUnit"
												default="0" />元
										</td>
										<td width="9%">认购总份数： <s:property
												value="#request.borrowDetailMap.circulationNumber"
												default="0" />份
										</td>
									</s:else>
									<s:if test="%{#request.borrowDetailMap.borrowStatus == 2}">
										<td width="7%">剩余时间：<span id="remainTimeOne"></span></td>
									</s:if>
								</tr>
								<tr>
									<td colspan="3">总投标数：<s:property
											value="#request.borrowDetailMap.investNum" default="0" />
										浏览量：<s:property value="#request.borrowDetailMap.visitors"
											default="---" /><br />
									<span style="color: red">${earnAmount}</span></td>
								</tr>
							</table>
						</div>
					</div>
				...
				<div class="reninfo">
					<div class="rinfomain">
						...
						<div class="name">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
									<td>用户名：${borrowUserMap.username} <s:if
											test="#request.borrowUserMap.vipStatus ==2"></s:if>
									</td>
								</tr>
								<tr>
									<td>居住地： ${borrowUserMap.address}</td>
								</tr>
								<tr>
									<td>信用指数： <img
										src="images/ico_${borrowUserMap.credit}.jpg"
										title="${borrowUserMap.creditrating}分" width="33" height="22" /></td>
								</tr>
								<tr>
								...
								<tr>
									<td align="center"><img src="images/neiye2_22.jpg"
										width="16" height="16" /> <a id="focusonUser"
										href="javascript:void(0);">关注此人</a></td>
								</tr>
							</table>
						</div>
					</div>
		...
		<div class="ifbox2">
			<div class="til"
				...
				<div class="til_bot"></div>
			</div>
			<div class="boxmain">
				<p class="xydj">
					富壹代信用等级：<img src="images/ico_${borrowUserMap.credit}.jpg"
						title="${borrowUserMap.creditrating}分" width="33" height="22" />
					<span>富壹代信用额度： ${borrowUserMap.creditLimit}</span>
				</p>
				<div class="tips">
					以下基本信息资料，经用户同意披露。其中<span class="fred">红色</span>字体的信息，为通过富壹代审核的项目。<br />
				...
				<div class="infotab">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							<td>性别：<s:if test="#request.borrowUserMap.auditperson == 3">
									<strong>${borrowUserMap.sex}</strong>
								</s:if>
								<s:else>${borrowUserMap.sex}</s:else></td>
							<td>年龄：<s:if test="#request.borrowUserMap.auditperson == 3">
									<strong>${borrowUserMap.age}</strong>
								</s:if>
								<s:else>${borrowUserMap.age}</s:else></td>
							<td colspan="3">婚姻状况：<s:if
									test="#request.borrowUserMap.auditperson == 3">
									<strong>${borrowUserMap.maritalStatus}</strong>
								</s:if>
								<s:else>${borrowUserMap.maritalStatus}</s:else></td>
							<td colspan="3">工作城市：<s:if
									test="#request.borrowUserMap.auditwork == 3">
									<strong>${borrowUserMap.workPro}&nbsp;${borrowUserMap.workCity}</strong>
								</s:if>
								<s:else>${borrowUserMap.workPro}&nbsp;${borrowUserMap.workCity}</s:else></td>
						</tr>
						<tr>
							<td>公司行业：<s:if test="#request.borrowUserMap.auditwork == 3">
									<strong>${borrowUserMap.companyLine}</strong>
								</s:if>
								<s:else>${borrowUserMap.companyLine}</s:else></td>
							<td>公司规模：<s:if test="#request.borrowUserMap.auditwork == 3">
									<strong>${borrowUserMap.companyScale}</strong>
								</s:if>
								<s:else>${borrowUserMap.companyScale}</s:else></td>
							<td colspan="3">职位：<s:if
									test="#request.borrowUserMap.auditwork == 3">
									<strong>${borrowUserMap.job}</strong>
								</s:if>
								<s:else>${borrowUserMap.job}</s:else></td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
						<tr>
							<td>毕业学校：<s:if test="#request.borrowUserMap.auditwork == 3">
									<strong>${borrowUserMap.school}</strong>
								</s:if>
								<s:else>${borrowUserMap.school}</s:else></td>
							...
							<td colspan="3">入学年份：<s:if
									test="#request.borrowUserMap.auditwork == 3">
									<strong>${borrowUserMap.eduStartDay}</strong>
								</s:if>
								<s:else>${borrowUserMap.eduStartDay}</s:else></td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
						<tr>
				...
				<div class="infotab">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<th colspan="5" align="left">富壹代借款记录</th>
						</tr>
						<tr>
							<td>发布借款标的：<s:if
									test="#request.borrowRecordMap.publishBorrow !=''">${borrowRecordMap.publishBorrow}</s:if>
								<s:else>0</s:else></td>
							<td>成功借款笔数：<s:if
									test="#request.borrowRecordMap.successBorrow !=''">${borrowRecordMap.successBorrow}</s:if>
								<s:else>0</s:else></td>
							<td>还清笔数：<s:if
									test="#request.borrowRecordMap.repayBorrow !=''">${borrowRecordMap.repayBorrow}</s:if>
								<s:else>0</s:else></td>
							<td>逾期次数：<s:if
									test="#request.borrowRecordMap.hasFICount !=''">${borrowRecordMap.hasFICount}</s:if>
								<s:else>0</s:else></td>
							<td>严重逾期次数：<s:if
									test="#request.borrowRecordMap.badFICount !=''">${borrowRecordMap.badFICount}</s:if>
								<s:else>0</s:else></td>
						</tr>
						<tr>
							<td>共借入：<s:if
									test="#request.borrowRecordMap.borrowAmount !=''">${borrowRecordMap.borrowAmount}</s:if>
								<s:else>0</s:else>
							</td>
							<td>待还金额：<s:if
									test="#request.borrowRecordMap.forRepayPI !=''">${borrowRecordMap.forRepayPI}</s:if>
								<s:else>0</s:else></td>
							<td>逾期金额：<s:if test="#request.borrowRecordMap.hasI !=''">${borrowRecordMap.hasI}</s:if>
								<s:else>0</s:else></td>
							<td>共借出：<s:if
									test="#request.borrowRecordMap.investAmount !=''">${borrowRecordMap.investAmount}</s:if>
								<s:else>0</s:else></td>
		...
		<div class="ifbox2">
			...
			<div class="boxmain">
				<div class="box">
					<div class="tips">富壹代将以客观、公正的原则，最大程度地核实借入者信息的真实性，但不保证审核信息100%真实。如果借入者长期逾期，其提供的信息将被公布。
					</div>
								...
								<s:iterator value="#request.list" id="bean">
									<tr>
                                     ...
                                     等待资料上传
										...
										<td align="center" class="ck"><s:if
												test="#bean.visiable== 1 && #bean.auditStatus==3">
												<a
			...
			<div class="ifbox2">
				...
				<div class="boxmain">
					<div class="">
						<strong>我的借款描述：</strong><br />
						<s:property value="#request.borrowDetailMap.borrowInfo" default=""
							escape="false" />
					</div>
			...
			<div class="ifbox2">
				...
				<div class="boxmain">
					<div class="tzjl">
						<ul>
							<li>目前总投标金额：<strong>￥${borrowDetailMap.hasInvestAmount}</strong></li>
							<li>剩余投标金额：<strong>￥${borrowDetailMap.investAmount}</strong></li>
							<s:if test="%{#request.borrowDetailMap.borrowStatus == 2}">
			...
			$('#invest')
					.click(
							function() {
								...
								if (step < 5) {
									window.location.href = 'financeInvestInit.do?id=${borrowDetailMap.id}';
									return false;
								}
								$
										.shovePost(
												'financeInvestInit.do',
												param,
												function(data) {
