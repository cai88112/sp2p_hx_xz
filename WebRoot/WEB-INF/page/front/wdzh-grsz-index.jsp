		...
		<div class="wdzh">
			...
			<div class="r_main">
				<div class="tabtil" style="width: 778px">
					<ul>
						<li class="on" dataIndex="true"
							onclick="jumpUrl('owerInformationInit.do');">个人详细信息</li>
				...
				<div class="box" id="baseinfo">
					<div class="boxmain2">
						<p class="tips">
							<span class="fred">*</span> 为必填项，所有资料均会严格保密。<span
								style="color: #DB7017">*</span> &nbsp;成为借款人必填项。
						</p>
						<div class="biaoge2">
							<form id="BaseDataform">
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									...
									<tr style="position: absolute; top: 95px">
										<td align="right"><span style="color: red; float: none;"
											class="formtips">*</span>身份证号：</td>
										<td><s:if test='#request.flag=="2"'>
									...
									<tr style="position: absolute; top: 120px">
										<td align="right"><span style="color: red; float: none;"
											class="formtips">*</span>手机号码：</td>
										<td><s:if test="#request.map.cellPhone!=null">
												<a id="cellPhone1"></a>
												<input type="hidden" class="inp188"
									...
									<s:else>
										<s:if test='#request.flag=="2"'>									
											<tr style="position: absolute; top: 147px">
													<td align="right">
													<span
														style="color: red; float: none;" class="formtips">*</span>手机验证码：
														
														
													</td>
													<td><input type="text" class="inp100x"	name="paramMap.vilidataNum" id="vilidataNum" /></td>
													
													<td>
														 
														<s:if test="#request.ISDEMO==1">
															<span style="color: red; float: none;" id="u_vilidat2"
																class="formtips"> * 演示站点测试不发送验证码，随意填写. </span>
														</s:if> <span style="color: red; float: none;" id="u_vilidataNum"
														class="formtips"> </span>
													</td>
												</tr>
											</s:if>
									...
									<tr style="position: absolute; top: 174px; left: 36px">
										<td align="right">性别：</td>
										<td>
											<%-- 	<s:if test='#request.flag=="2"'>--%> <input
											type="radio" id="sex" value="男" name="paramMap_sex"
											<s:if test='#request.map.sex == "男"'>checked="checked"</s:if>
											class="sex" onclick="javascript:$('#u_sex').html('')"
											<s:if test='#request.map.auditStatus ==3'>disabled="disabled"</s:if> />
											男 <input type="radio" id="sex" value="女" name="paramMap_sex"
											<s:if test='#request.map.sex == "女"'>checked="checked"</s:if>
											class="sex" onclick="javascript:$('#u_sex').html('')"
											<s:if test='#request.map.auditStatus ==3'>disabled="disabled"</s:if> />
											女 <span style="color: red; float: none;" id="u_sex"
											class="formtips"></span> <%-- 	
					</s:if>
						<s:else>
												        ${map.sex }		
					...
					<td class="tishi">
						...
						一旦被发现所填资料有虚假，将会影响您在富壹代的信用，对以后借款造成影响。
									...
									<tr>
										<td align="right">&nbsp;</td>
										<td
											style="padding-top: 20px; position: absolute; top: 200px; left: 100px;">
											<!--  
												<s:if test="#session.user.map.auditStatus==3"></s:if>
												<s:else>
												--> <input type="button" value="保存" class="bcbtn"
											id="jc_btn"
											style="background-color: #af95d2; border-radius: 3px; background-image: none; color: #fff; font-size: 14px; height: 28px; line-height: 28px"
											<s:if test='#request.map.auditStatus ==3'>disabled="disabled"</s:if> />
											<!--</s:else>	-->
										</td>
									</tr>
	...
	<script type="text/javascript">
		...
		function registedPlaceCity(parentId, regionType) {
			...
			$.post("ajaxqueryRegion.do", param, function(data) {
				for (var i = 0; i < data.length; i++) {
					_array.push("<option value='"+data[i].regionId+"'>"
		...
		$("#jc_btn").click(
				function() {
					...
					if (tsex == '') {
						param["paramMap.sex"] = $(
								"input[name='paramMap_sex']:checked").val();
					} else {
					...
					param["paramMap.maritalStatus"] = $(
							"input[name='paramMap_maritalStatus']:checked")
							.val();
					...
					param["paramMap.hasHousrseLoan"] = $(
							"input[name='paramMap_hasHousrseLoan']:checked")
							.val();
					param["paramMap.hasCar"] = $(
							"input[name='paramMap_hasCar']:checked").val();
					...
					param["paramMap.registedPlacePro"] = $("#registedPlacePro")
							.val();
					param["paramMap.registedPlaceCity"] = $(
							"#registedPlaceCity").val();
