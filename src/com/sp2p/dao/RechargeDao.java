...
public class RechargeDao {
	...
	public long addRecharge(Connection conn, Map<String, String> paramMap)
		...
		DBReflectHelper.mapToTableValue(t_recharge, paramMap);
	...
	public long addRechargeDetail(Connection conn, Map<String, String> paramMap)
			throws SQLException {
		Dao.Tables.t_recharge_detail t_recharge_detail = new Dao().new Tables().new t_recharge_detail();
		DBReflectHelper.mapToTableValue(t_recharge_detail, paramMap);
	...
	public Map<String, String> queryMoneyRecords(Connection conn, long userId)
		...
		command = null;
		return DataSetHelper.dataSetToMap(dds);
	}
	...
	* @throws
	 */
	public Map<String, String> getWithdrawByWithdrawId(Connection conn, long wid)
			throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		command
				.append("select w.id as wid,w.acount,w.`status`, w.sum, w.poundage,bc.bankName,bc.bankCode,bc.cardNo,bc.branchBankName,bc.cityCode,bc.provinceCode,bc.cardType ");
		command
				.append(" from t_withdraw w LEFT JOIN t_bankcard bc ON w.bankId = bc.id where w.id = "+wid+";");
