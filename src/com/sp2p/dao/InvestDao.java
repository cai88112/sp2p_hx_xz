...
public class InvestDao {
	 ...
	 * @throws SQLException
	 */
	public long addInvestHistory(Connection conn, Map<String, String> paramMap)
			throws SQLException {
		Dao.Tables.t_invest_history t_invest_history = new Dao().new Tables().new t_invest_history();
		DBReflectHelper.mapToTableValue(t_invest_history, paramMap);
	 ...
	 */
	public Map<String, String> getInvest(Connection conn, long id)
	 ...
	 * @throws SQLException
	 */
	public long updateInvest(Connection conn, long id,
			Map<String, String> paramMap) throws SQLException {
		Dao.Tables.t_invest t_invest = new Dao().new Tables().new t_invest();
		DBReflectHelper.mapToTableValue(t_invest, paramMap);
	...
	public List<Map<String, Object>> queryInvestInfoById(Connection conn,
		...
		command.append(" left join t_user b on a.investor=b.id  where a.borrowId="
				+ borrowId);
	 ...
	 * @Throws:
	 */
	public List<Map<String, Object>> queryInvestLoanNoByBorrowId(Connection conn,
			long borrowId) throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		command.append("select a.id,a.investAmount,a.investor,a.loanNo from t_invest a");
		command.append(" where a.borrowId="
				+ borrowId);
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		return dataSet.tables.get(0).rows.rowsMap;
	 ...
	 * @Throws:
	 */
	public Map<String, String> queryBorrowMany(Connection conn, long borrowId)
			throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		...
		command.append(" FROM t_borrow a LEFT JOIN t_user b ON a.publisher = b.id LEFT JOIN t_person f ON f.userId = b.id LEFT JOIN t_select s ON s.selectValue = a.purpose  LEFT JOIN t_repayment  tt on tt.borrowId = a.id  WHERE  a.id ="
				+ borrowId + " and s.typeId =1 ");
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		command = null;
	 ...
	 * @throws SQLException
	 */
	public List<Map<String, Object>> queryInvestMomey(Connection conn,
			long invest_id, long borrowId) throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		...
		command.append(" FROM  t_repayment a LEFT JOIN t_invest_repayment b  ON a.id = b.repayId   LEFT JOIN t_user u  on u.id = b.`owner` LEFT JOIN  t_borrow  tb on tb.id = a.borrowId WHERE  a.borrowId ="
				+ borrowId + "  ");
		if (invest_id > 0) {
	 ...
	 * @throws SQLException
	 */
	public List<Map<String, Object>> queryUsername(Connection conn,
			long borrowId, long invest_id) throws SQLException, DataException {

		StringBuffer command = new StringBuffer();
		command.append(" SELECT u.username as username,f.realName as realName   FROM   t_repayment a LEFT JOIN t_invest_repayment b  ON a.id = b.repayId    LEFT JOIN t_user u  on u.id = b.`owner` LEFT JOIN t_person f ON f.userId =u.id  LEFT JOIN  t_borrow  tb on tb.id = a.borrowId WHERE  a.borrowId = "
				+ borrowId);
		if (invest_id > 0) {
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryBorrowSumMomeny(Connection conn,
			long borrowId, long invest_id) throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		...
		command.append(" FROM  t_repayment a LEFT JOIN t_invest_repayment b ON a.id = b.repayId  WHERE  a.borrowId = "
				+ borrowId + "");
		if (invest_id > 0) {
			command.append(" and b.invest_id=  " + invest_id);
		}
	 ...
	 * @Throws:
	 */
	public long addInvestRepayment(Connection conn, long repayId,
			String repayPeriod, String repayDate, double recivedPrincipal,
			double recivedInterest, double principalBalance,
			double interestBalance, long invest_id, long owner,
			String ownerlist, double iManageFeeRate, double hasPrincipal,
			double hasInterest, double iManageFee) throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public long updateHasIntert(Connection conn, long repayId, long investId,
			long owner, double getInterest) throws SQLException {
		Dao.Tables.t_invest_repayment t_invest_repayment = new Dao().new Tables().new t_invest_repayment();
		StringBuffer command = new StringBuffer();
	...
	public long updateInvestRepayment(Connection conn, long repayId,
			long investId, long owner, double lateFI, long interestOwner,
			long isWebRepay) throws SQLException {
		StringBuffer command = new StringBuffer();
	...
	public long updateInvestRepayment(Connection conn, long repayId,
			long investId, long owner, double lateFI, long interestOwner,
			long isWebRepay, double hasP, double hasI) throws SQLException {
		StringBuffer command = new StringBuffer();
		SimpleDateFormat sf = new SimpleDateFormat(DateHelper.UNDERLINE_DATE_SHORT);
		command.append("update t_invest_repayment SET hasPrincipal = " + hasP
				+ ",hasInterest=" + hasI + ",");
		command.append("realRepayDate='" + sf.format(new Date()) + "'");
		command.append(",interestOwner = " + interestOwner);
		command.append(",isWebRepay = " + isWebRepay);
	 ...
	 * @Throws:
	 */
	public long updateInvestDebtStatus(Connection conn, long investId,
			long owner) throws SQLException {
		String command = "update t_invest_repayment SET owner = " + owner
				+ ",ownerlist=concat(ownerlist,'," + owner
				+ "'),isDebt=2 where invest_id=" + investId
				+ " and repayStatus=1";
		return MySQL.executeNonQuery(conn, command);
	}

	public Map<String, String> queryInvestRepayment(Connection conn,
			long investId, long repayId) throws DataException, SQLException {
		StringBuffer command = new StringBuffer();
		command.append(" select hasPrincipal as hhap, hasInterest as hhaI from t_invest_repayment ");
		command.append(" where   invest_id = " + investId + " and repayId = "
				+ repayId + " ");
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		return DataSetHelper.dataSetToMap(dataSet);
	 ...
	 * @throws SQLException
	 */
	public long addInvestRepayment_(Connection conn, long repayId,
			String repayPeriod, String repayDate, double recivedPrincipal,
			double recivedInterest, double principalBalance,
			double interestBalance, long invest_id, long owner,
			String ownerlist, double iManageFeeRate, double hasPrincipal,
	 ...
	 * @Throws:
	 */
	public List<Map<String, Object>> queryInvestorByBorrowId(Connection conn,
	 ...
	 * @Throws:
	 */
	public long updateInvestRepayStatus(Connection conn, long investId)
			throws SQLException {
		long returnId = -1;
		SimpleDateFormat sf = new SimpleDateFormat(DateHelper.UNDERLINE_DATE_SHORT);
	...
	public long updateInvestRepaymentStatus(Connection conn, long invest_id)
			throws SQLException {
		String command = "update t_invest_repayment  set repayStatus = 2 , circulationForpayStatus =2  where invest_id =  "
				+ invest_id;
		return MySQL.executeNonQuery(conn, command);
	}
	 ...
	 * @throws SQLException
	 */
	public long addFlowRepayment(Connection conn, long invest_id)
			throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public Map<String, String> queryFlowMap(Connection conn, long invest_id)
			throws DataException, SQLException {
		Dao.Tables.t_flow_repayment t_flow_reRepayment = new Dao().new Tables().new t_flow_repayment();
		DataSet ds = t_flow_reRepayment.open(conn, " * ", " invest_id =  "
				+ invest_id, "", -1, -1);
	 ...
	 * @throws SQLException
	 */
	public long updateInvesthasPrincipalAndhasInterest(Connection conn,
			long investid, double hasPrincipal, double hasInterest)
			throws SQLException {
		long returnId = -1;
		StringBuffer sb = new StringBuffer();
		sb.append("update t_invest set hasPrincipal  =   " + hasPrincipal
				+ ",t_invest.hasInterest = " + hasInterest + " where id = "
				+ investid);
	 ...
	 * @throws SQLException
	 */
	public Map<String, String> queryInvestId(Connection conn, long borrowId,
			long userId) throws SQLException, DataException {
		Dao.Tables.t_invest t_invest = new Dao().new Tables().new t_invest();
		DataSet ds = t_invest.open(conn, "  max(id) as investId  ",
				" borrowId = " + borrowId + "  and  investor = " + userId, "",
				-1, -1);
		return DataSetHelper.dataSetToMap(ds);
	}
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryInvestSumInPeriod(Connection conn,
			long userId, String timeStart, String timeEnd) throws DataException {
		Dao.Tables.t_invest t_invest = new Dao().new Tables().new t_invest();

		DataSet ds = t_invest
				.open(conn,
						"  SUM(recivedPrincipal) as sum  ",
						" investor ="
								+ userId
								+ " and investTime < '"
								+ timeEnd
								+ "' and investTime > '"
								+ timeStart
								+ "' and borrowId NOT IN(SELECT id FROM t_borrow  WHERE isDayThe = 2  and  id IN(select borrowId from t_invest where investor ="
								+ userId + " and investTime < '" + timeEnd
								+ "'));", "", -1, -1);
		return DataSetHelper.dataSetToMap(ds);
	}
	 ...
	 * @throws DataException
	 */
	public List<Map<String, Object>> queryInvestSumInPeriod(Connection conn,
			String timeStart, String timeEnd) throws DataException {
		Dao.Tables.t_invest t_invest = new Dao().new Tables().new t_invest();

		DataSet ds = t_invest.open(conn, "  SUM(recivedPrincipal) as sum  ",
				" investTime < '" + timeEnd + "' and investTime > '"
						+ timeStart + "';", "", -1, -1);
	 ...
	 * @return
	 */
	public List<Map<String, Object>> queryInvestSumInterest(Connection conn)
			throws DataException {
		Dao.Tables.t_invest t_invest = new Dao().new Tables().new t_invest();

		DataSet ds = t_invest.open(conn, "  SUM(recievedInterest) as sum  ",
				"", "", -1, -1);
		ds.tables.get(0).rows.genRowsMap();
		return ds.tables.get(0).rows.rowsMap;
	}
