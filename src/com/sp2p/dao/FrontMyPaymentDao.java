...
public class FrontMyPaymentDao {
	 ...
	 */
	public List<Map<String, Object>> queryRepay(Connection conn, long repayId)
			throws SQLException, DataException {
		String command = "select ti.investAmount,tir.recivedPrincipal,tir.recivedInterest,tir.iManageFee,tuser.portMerBillNo,tir.owner,tir.recivedFI,ti.investor "
				+ " from t_invest_repayment tir left join t_user tuser on tir.owner = tuser.id "
				+ " left join t_invest ti on tir.invest_id=ti.id where repayId = "
				+ repayId;
	 ...
	 * @throws DataException
	 */
	public List<Map<String, Object>> queryInvestByBorrowId(Connection conn,
			long borrowId) throws SQLException, DataException {
		String command = "SELECT ti.id,ti.investor,trem.sumreturnedmoney,ti.investAmount from t_invest ti join t_returnedmoney trem on ti.investor = trem.userId WHERE ti.borrowId="
				+ borrowId;
		DataSet dataSet = MySQL.executeQuery(conn, command);
		dataSet.tables.get(0).rows.genRowsMap();
		command = null;
	...
	public Map<String, String> queryOneBorrowInfo(Connection conn, long userId,
			long borrowId, int limitStart, int limitCount) throws SQLException,
			DataException {
		Dao.Views.t_borrow_success_list t_info = new Dao().new Views().new t_borrow_success_list();
		DataSet dataSet = t_info.open(conn, "*", " publisher=" + userId
				+ " and borrowId=" + borrowId, "", limitStart, limitStart);
	 ...
	 * @throws DataException
	 */
	public List<Map<String, Object>> queryPayingDetails(Connection conn,
			long borrowId, int limitStart, int limitCount) throws SQLException,
			DataException {
		String condition = " borrowId=" + borrowId + " and repayStatus!="
	...
	public List<Map<String, Object>> queryPayingBorrowIds(Connection conn,
			long userId) throws SQLException, DataException {
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryMyPayData(Connection conn, long payId,
			int limitStart, int limitCount) throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		command.append("SELECT a.id,a.borrowId, (c.usableSum+c.freezeSum) totalSum,c.usableSum,DATE_FORMAT(a.repayDate,'%Y-%m-%d') repayDate,");
		...
		command.append(" FROM t_repayment a LEFT JOIN t_borrow b ON a.borrowId=b.id LEFT JOIN t_user c ON b.publisher=c.id where a.id="
				+ payId + " limit 0,1");
	 ...
	 * @Throws:
	 */
	public Map<String, String> queryRepayAmountById(Connection conn, long id)
			throws SQLException, DataException {
		String command = "SELECT stillPrincipal from t_repayment where id="
				+ id + " limit 0,1";
		DataSet dataSet = MySQL.executeQuery(conn, command);
		command = null;
		return DataSetHelper.dataSetToMap(dataSet);
	 ...
	 * @Throws:
	 */
	public Map<String, String> queryRepayBorrow(Connection conn, long id)
			throws SQLException, DataException {
		String command = "SELECT id ,borrowId FROM t_repayment WHERE repayStatus = 1 and id ="
				+ id + " limit 0,1";
		DataSet dataSet = MySQL.executeQuery(conn, command);
		command = null;
		return DataSetHelper.dataSetToMap(dataSet);
	 ...
	 * @Throws:
	 */
	public Map<String, String> isRepayBorrow(Connection conn, long id)
			throws SQLException, DataException {
		String command = "SELECT (stillPrincipal+stillInterest-hasPI) as needPI,lateFI,stillPrincipal,borrowId FROM t_repayment WHERE id ="
	 ...
	 * @Throws:
	 */
	public Map<String, String> userAmountMap(Connection conn, double needSum,
			long userId) throws DataException, SQLException {
		String command = "SELECT id FROM t_user WHERE usableSum < " + needSum
				+ " and id =" + userId;
		DataSet dataSet = MySQL.executeQuery(conn, command);
		command = null;
		return DataSetHelper.dataSetToMap(dataSet);
	 ...
	 * @Throws:
	 */
	public Map<String, String> validatePassWord(Connection conn, long userId,
			String dealPWD) throws SQLException, DataException {
		String command = "SELECT dealpwd  FROM t_user where dealpwd = '"
				+ StringEscapeUtils.escapeSql(dealPWD) + "' and id =" + userId
				+ " and dealpwd is not null";
		DataSet dataSet = MySQL.executeQuery(conn, command);
		command = null;
		return DataSetHelper.dataSetToMap(dataSet);
	}
	 ...
	 * @Throws:
	 */
	public Map<String, String> queryHasDeadLine(Connection conn, long id)
			throws SQLException, DataException {
		// ----modify by houli 添加isDayThe 字段的查询
		String command = "SELECT borrowAmount AS borrowAmount, id as borrowId,hasDeadline as hasDeadline,borrowWay as borrowWay,isDayThe as isDayThe,borrowTitle as borrowTitle,borrowShow FROM t_borrow where id = (select borrowId from t_repayment where id="
				+ id + ")";
	 ...
	 * @Throws:
	 */
	public Map<String, String> isRepay(Connection conn, int hasDeadline, long id)
			throws SQLException, DataException {
		String command = "SELECT id FROM t_borrow WHERE deadline > "
				+ hasDeadline
				+ " and id = (select borrowId from t_repayment where id=" + id
				+ ")";
		DataSet dataSet = MySQL.executeQuery(conn, command);
		command = null;
		return DataSetHelper.dataSetToMap(dataSet);
	 ...
	 * @Throws:
	 */
	public long updateRepayedBorrow(Connection conn, int hasDeadline, long id,
			int sorts) throws SQLException {
		String command = "update t_borrow set borrowStatus = 5 ,sort = "
				+ sorts
				+ " where deadline <= "
				+ hasDeadline
				+ " and borrowStatus = 4 and id = (select borrowId from t_repayment where id="
				+ id + ")";
	...
	/**
	 * add by houli 处理天标还款时的状态，天标只需要还一期
	 * 
	 * @param conn
	 * @param id
	 * @return
	 * @throws SQLException
	 */
	public long updateRepayedBorrow(Connection conn, long id, int sorts)
			throws SQLException {
		String command = "update t_borrow set borrowStatus = 5 ,sort = "
				+ sorts
				+ "  where hasDeadline  = 1 and borrowStatus = 4 and id = (select borrowId from t_repayment where id="
				+ id + ")";
		long result = MySQL.executeNonQuery(conn, command);
		command = null;
		return result;
	 ...
	 * @Throws:
	 */
	public Map<String, String> isWebRepay(Connection conn, long id)
			throws SQLException, DataException {
		String command = "SELECT isWebRepay,borrowId FROM t_repayment where id="
				+ id;
		DataSet dataSet = MySQL.executeQuery(conn, command);
		command = null;
		return DataSetHelper.dataSetToMap(dataSet);
	...
	public long updateRepayInfo(Connection conn, double needPI, double lateFI,
		...
		command.append(",realrepaydate=DATE_FORMAT('" + sf.format(new Date())
				+ "','%Y-%m-%d %H:%i:%S')");
	 ...
	 * @Throws:
	 */
	public long updateBorrowInfo(Connection conn, long borrowId)
			throws SQLException {
		String command = "update t_borrow set hasDeadline = hasDeadline+1 where id ="
				+ borrowId;
		long result = MySQL.executeNonQuery(conn, command);
		command = null;
	 ...
	 * @Throws:
	 */
	public long updateUserAmount(Connection conn, long userId, double needSum)
			throws SQLException {
		String command = "update t_user set usableSum = usableSum - " + needSum
				+ " where id =" + userId;
		long result = MySQL.executeNonQuery(conn, command);
		command = null;
		return result;
	...
	public long rebackUserCreditLimit(Connection conn, double borrowAmount,
		...
		StringBuffer command = new StringBuffer();
		command.append(" update t_user set usableCreditLimit = usableCreditLimit+"
				+ borrowAmount);
	 ...
	 * @Throws:
	 */
	public List<Map<String, Object>> queryInvestors(Connection conn, long id)
			throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		...
		command.append(" ON a.borrowId = b.id LEFT JOIN t_repayment c ON c.borrowId = b.id left join t_user d on a.investor=d.id WHERE c.id = "
				+ id);
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		command = null;
	...
	public long updateInvestRecord(Connection conn, double hasP, double hasI,
		...
		StringBuffer command = new StringBuffer();
		command.append(" update t_invest set hasPrincipal = hasPrincipal + "
				+ hasP + ", hasInterest=hasInterest+" + hasI);
		command.append(" ,recivedFI=recivedFI+" + hasLFI
				+ ",manageFee =manageFee+" + mFee + " where id =" + investId);
	 ...
	 * @Throws:
	 */
	public long updateInvestRepay(Connection conn, long investId)
			throws SQLException {
		StringBuffer command = new StringBuffer();
		command.append("update t_invest set repayStatus = 2 where round(recivedPrincipal+recievedInterest,2)=round(hasPrincipal+hasInterest,2) and id ="
				+ investId);
		long result = MySQL.executeNonQuery(conn, command.toString());
		command = null;
		return result;
	 ...
	 * @Throws:
	 */
	public long updateInvestorSum(Connection conn, double hasSum, long investor)
			throws SQLException {
		StringBuffer command = new StringBuffer();
		command.append("update t_user set usableSum = usableSum + " + hasSum
				+ " where id =" + investor);
		long result = MySQL.executeNonQuery(conn, command.toString());
		command = null;
		return result;
	 ...
	 * @Throws:
	 */
	public long closeAutoBid(Connection conn, long investor)
			throws SQLException {
		StringBuffer command = new StringBuffer();
		command.append("update t_automaticbid set  bidStatus = 1 where userId ="
				+ investor);
		long result = MySQL.executeNonQuery(conn, command.toString());
		command = null;
		return result;
	 ...
	 * @Throws:
	 */
	public Map<String, String> queryOverdueInfo(Connection conn, long id)
			throws DataException, SQLException {
		DataSet dataSet = MySQL
				.executeQuery(
						conn,
						"select a.lateDay,b.borrowWay,isDayThe,b.nid_log from t_repayment a left join t_borrow b on a.borrowId=b.id where a.id ="
								+ id);
		return DataSetHelper.dataSetToMap(dataSet);
	}
	 ...
	 * @Throws:
	 */
	public long deducatedUserAmount(Connection conn, long userId, double amount)
			throws SQLException {
		long returnId = -1;
		returnId = Database.executeNonQuery(conn,
				" update t_user set usableSum=usableSum-" + amount
						+ " where id =" + userId);
		return returnId;
	}
}
