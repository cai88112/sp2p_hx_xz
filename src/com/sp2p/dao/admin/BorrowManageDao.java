...
//		command.append("update t_borrow set remainTimeStart = '"+sf.format(new Date())+"',remainTimeEnd = date_add('"+sf.format(new Date())+"',INTERVAL raiseTerm day) where id ="+id);
...
//		sf=null;
	...
	*/
	public Map<String, String> queryBorrowInfo(Connection conn, long idLong) throws SQLException, DataException {
		...
		command.append(" a.annualRate,a.borrowTitle,b.username ,a.frozen_margin,a.feestate,a.feelog , a.tradeNo as tradeNo, a.guaranteeNo as guaranteeNo, ");
		command.append(" d.realName as realName, d.idNo as idNo, b.portMerBillNo as portMerBillNo");
...
//			DataSet dataSet = t_borrow.open(conn, " publisher,borrowTitle,annualRate,paymentMode,borrowAmount,deadline,borrowWay,excitationType,excitationSum,isDayThe ", "  id="+id, "", 0, 1);
		...
		*/
		...
		/**
		 * 根据借款id查询借款标id,version,deadline,annualRate信息.
		 * @param conn
		 * @param borrowId
		 * @return
		 * @throws DataException
		 * @throws SQLException
		 */
		public Map<String, String> queryBorrowInfoById(Connection conn, long borrowId)throws DataException, SQLException{
			String command = "SELECT id,version,deadline,annualRate,borrowTitle,isDayThe,subject_activity FROM `t_borrow` where id="+borrowId+";";
			DataSet dataSet = MySQL.executeQuery(conn, command);
			return DataSetHelper.dataSetToMap(dataSet);
		}
