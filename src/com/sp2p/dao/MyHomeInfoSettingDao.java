...
public class MyHomeInfoSettingDao {
	...
	public static Log log = LogFactory.getLog(MyHomeInfoSettingDao.class);
	/**
	 * 该用户密码
	 * @param conn
	 * @param userId
	...
	public Long addBankCardInfo(Connection conn,long userId,String cardUserName,String bankName,String subBankName,
			String bankCard,int cardStatus,String bankCode,String provinceCode,String cityCode,String cardType) throws SQLException{
		...
		t_bank.bankCode.setValue(bankCode);//银行卡编码
		t_bank.provinceCode.setValue(provinceCode);
		t_bank.cityCode.setValue(cityCode);
		t_bank.cardType.setValue(cardType);
   ...
   * @Title: quePortUserId 
   * @param @param conn
   * @param @param hxUserId
   * @param @param pMerBillNo
   * @param @return
   ...
   * @throws
    */
	public Map<String, String> quePortUserId(Connection conn, String hxUserId, String pMerBillNo) throws SQLException, DataException {
		Dao.Tables.t_hx_user t_hx_user = new Dao().new Tables().new t_hx_user();
		StringBuffer condition = new StringBuffer();
		condition.append(" 1 = 1 ");
		if(hxUserId != null && StringUtils.isNotBlank(hxUserId)){
			condition.append(" and hxUserId = " + hxUserId);
		}
		if(pMerBillNo != null && StringUtils.isNotBlank(pMerBillNo)){
			condition.append(" and orderNo = " + pMerBillNo);
		}
		DataSet dataSet = t_hx_user.open(conn, " * ", condition.toString(), "", -1, -1);
