...
public class AuctionDebtDao {
	...
	public long addAuctionDebt(Connection conn, Map<String, String> paramMap) throws SQLException {
		...
		DBReflectHelper.mapToTableValue(t_auction_debt, paramMap);
	...
	public List<Map<String, Object>> queryAuctionDebtByDebtId(Connection conn,
		...
		DataSet ds = v_t_auction_debt_user.open(conn, " id  ,  debtId  , auctionTime  ,   auctionPrice  ,f_formatting_username(username) as username  , userId   ", "debtId="+debtId, "", -1, -1);
	...
	public Map<String, String> queryAuctionMaxPriceAndCount(Connection conn,long debtId) throws SQLException, DataException {
		...
		if(list != null && list.size() > 0){
			...
			map.put("auctionCount",DataSetHelper.dataSetToMap(ds).get("counts"));
		}
		list=null;
		conditon=null;
		return map;
	...
	public long queryBorrowerByBorrowId(Connection conn, long borrowId) throws SQLException, DataException {
		...
		return Convert.strToLong(DataSetHelper.dataSetToMap(ds).get("publisher"),-1);
	}
	...
	public long queryBorrowIdByRepayId(Connection conn,long repayId) throws SQLException, DataException {
		...
		return Convert.strToLong(DataSetHelper.dataSetToMap(ds).get("borrowId"),-1);
	}
