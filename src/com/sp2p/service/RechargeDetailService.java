...
public class RechargeDetailService extends BaseService {
	...
	public int usergoPay(Map<String, Object> paramMap) throws Exception {
		...
		try {
			...
			Date notify_time = DateUtils.parseDate(notify_timeStr,new String[]{"yyyyMMddHHmmssSSS"});
			log.info("3-");
		...
		try {
			...
			if (detailMap != null) {
				if ("1".equals(detailMap.get("result"))) {
					...
					// 修改充值详细状态
					String rechargeTime = DateFormatUtils.format(notify_time, "yyyy-MM-dd HH:mm:ss");
					...
					if (resultCount > 0) {
						...
						fundMap.put("recordTime", DateFormatUtils.format(new Date(), "yyyy-MM-dd HH:mm:ss"));
