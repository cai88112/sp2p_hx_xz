...
public class ValidateService extends BaseService {
	...
	public Long updateUserCreditLimit(Long userId, Integer Creditstatus,
		...
		try {
			...
			if (resultId > 0) {
				...
				// 发站内信
				resultId = sendmsgService.sendCheckMail(userId, " 信用额度审核通知",
	...
	public Long Updatecreditrating(Long userId, String alloption,
		...
		try {
			...
			resultId = sendmsgService.sendCheckMail(userId, " 基本资料审核通知", msg
	...
	public Long Updatematerialsauth(Long mtdId, Long userId,
		...
		try {
			resultId = vidateDao.Updatematerialsauth(conn, mtdId, userId,
					materAuthTypeId, option, auditStatus, materaldetalId,
					visiable);
			if (resultId <= 0) {
				conn.rollback();
				return -1L;
	...
	public Long updateworkStatus(Long userId, Long workauthId,
		...
		try {
			...
			if (flag1) {// 表示审核通过了 要发送站内信
				...
				// 发站内信
				resultId = sendmsgService.sendCheckMail(userId, " 工作信息的审核通知",
						msg.toString(), 2, adminId);// 2管理员信件
				if (resultId <= 0) {
					conn.rollback();
					return -1L;
