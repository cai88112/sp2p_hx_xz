...
public class ShoveBorrowStyleService extends BaseService {
	...
	public List<Map<String, Object>> queryShoveBorrowStyleByTypeNid(String nid)
		...
		try {
			String styles = shoveBorrowTypeDao.queryOneByNid(conn, "styles",
					nid);
			if (styles != null && styles.length() > 0) {
				mapList = shoveBorrowStyleDao.queryShoveBorrowStyleByTypeNid(
						conn, styles);
			} else {
	 ...
	 * @throws DataException
	 */
	public Map<String, String> querySlectStyleByTypeNid(String nid, int types)
			throws Exception {
		...
		try {
			if (types == 3) {
				// 反担保方式
				styles = shoveBorrowTypeDao.queryOneByNid(conn,
						"counter_guarantee", nid);
			}
			if (types == 2) {
				// 担保机构
				styles = shoveBorrowTypeDao.queryOneByNid(conn, "institution",
						nid);
			}
			if (styles != null && styles.length() > 0) {
	...
	public long updateShoveBorrowStyle(int id, int status, String title,
			String contents, String remark, int sort) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1L;
		try {
	 ...
	 * @throws SQLException
	 */
	public long deleteShoveBorrowStyle(int id) throws Exception {
		Connection conn = MySQL.getConnection();
		long result = -1L;
		try {
	 ...
	 * @throws SQLException
	 */
	public List<Map<String, Object>> queryBorrowAll() throws Exception {
		Connection conn = MySQL.getConnection();
		List<Map<String, Object>> mapList = null;
		try {
