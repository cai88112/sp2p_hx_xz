...
public class NewsAndMediaReportService extends BaseService {
	...
	public Long addNews(Integer sort, String title, String content,
			Long userId, String visits, String publishTime,String noticeType)
			throws Exception {
		Connection conn = MySQL.getConnection();
		Long result = 0L;
		try {
			result = newsDao.addNews(conn, sort, title, content, publishTime,
					userId, visits,noticeType);
	...
	public Long updateNews(Long id, Integer sort, String title, String content,
			Long userId, Integer visits, String publishTime,String noticeType)
			throws Exception {
		Connection conn = MySQL.getConnection();
		Long result = 0L;
		try {
			result = newsDao.updateNews(conn, id, sort, title, content,
					publishTime, userId, visits,noticeType);
	...
	public void frontQueryNewsPage(PageBean<Map<String, Object>> pageBean)
		...
		try {
			...
			dataPage(conn, pageBean, "v_t_news_list", "*",
					" order by publishTime desc ", " AND state=1 AND noticeType='00'");
		} catch (Exception e) {
	 ...
	 * @throws Exception
	 */
	public void frontQueryNewsMediaPage(PageBean<Map<String, Object>> pageBean)
			throws Exception {
		Connection conn = MySQL.getConnection();
		try {
			// modify by houli 按照后台排序顺序显示
			dataPage(conn, pageBean, "v_t_news_list", "*",
					" order by publishTime desc ", " AND state=1 AND noticeType='01'");
		} catch (Exception e) {
	...
	public void queryMediaReportPage(PageBean<Map<String, Object>> pageBean)
		...
		try {
			dataPage(conn, pageBean, " t_mediareport  ", " * ",
					" order by stick desc,sort desc ", condition + "");
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
			throw e;
	 ...
	 * @throws DataException
	 */
	public Map<String, String> getNewsById(Long id,int type) throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = null;
		try {
			map = newsDao.getNewsById(conn, id, type);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
