...
public class FinanceToolsService extends BaseService {
	...
	public List<Map<String, Object>> rateIncome2Month(double borrowSum,double yearRate,int borrowTime,
		 ...
		 * 年化收益
		 */
		double rateVal = ExcelFunctionHelper.excelRate((borrowSum - rewardSum), Double.parseDouble(df.format(monRepay)), monTime, 200, 10);
	...
	*/
	...
	public List<Map<String, Object>> rateIncome2Sum(double borrowSum,double yearRate,int borrowTime,
		 ...
		 * 年化收益
		 */
		double rateVal = ExcelFunctionHelper.rateTotal(allSum, (borrowSum - rewardSum), borrowTime);
	 ...
	 * @return
	 */
	public List<Map<String, Object>> rateCalculateSumOne(double borrowSum,double yearRate,int borrowTime,
			double bidReward,double bidRewardMoney){
		if(borrowSum < 0 || yearRate < 0 || borrowTime < 0){
			return null;
		}
