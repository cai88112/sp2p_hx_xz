...
public class JobTaskDao {
	...
	public List<Map<String, Object>> queryAllBider(Connection conn)
		...
		command.append("select id,borrowAmount,hasInvestAmount,borrowWay,isDayThe from v_t_autobid_borrow");
	 ...
	 * @Throws:
	*/
	public List<Map<String, Object>> queryUserBider(Connection conn,long borrowId,long userId)
			throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
	 ...
	 * @Throws:
	*/
	public Map<String, String> queryBorrowOwer(Connection conn,long borrowId,long userId)
			throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
	 ...
	 */
	public Map<String, String> queryAutoSign(Connection conn, long userId) throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		command.append("select id from t_user where id="+userId+" and (pIpsBillNo is null or pIpsBillNo=0)");
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		return DataSetHelper.dataSetToMap(dataSet);
	}
	...
	public Map<String, String> queryBiderByParam(Connection conn,
			...
			int deadLineEnd, int creditStart, int creditEnd, long id,int isDayThe)
			throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
	...
	*/
	public Map<String, String> queryBorrowInfo(Connection conn, int borrowId) throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		command.append("SELECT (annualRate/12) monthRate,deadline,borrowTitle,publisher,version  FROM t_borrow WHERE id= "
				+ borrowId + " limit 0,1");
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		return DataSetHelper.dataSetToMap(dataSet);
	}
	...
	*/
	 ...
	 * @Throws:
	*/
	public List<Map<String, Object>> queryFriendsvip2(Connection conn,long id) throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		command.append("SELECT tv.id from t_vipsum tv LEFT JOIN t_recommend_user tru ON tru.userId=tv.userId WHERE  tru.moneyTypeId="+id);
		DataSet dataSet = Database.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		return dataSet.tables.get(0).rows.rowsMap;
	}
	...
	*/
	public Map<String, String> queryRiskBalance(Connection conn) throws SQLException, DataException {
		...
		*/
		public List<Map<String, Object>> queryOverDueRepayment(Connection conn,String date) throws SQLException, DataException {
			StringBuffer command = new StringBuffer();
			command.append("select id,repayDate ,stillPrincipal ,stillInterest ,executeTime ,borrowId,DATEDIFF('"+date+"',executeTime) executeDay");
			command.append(",DATEDIFF('"+date+"',repayDate) overdueDay from t_repayment where ");
			command.append(" DATE_FORMAT(repayDate,'%Y-%m-%d') < '"+date+"' and repayStatus = 1");
			DataSet dataSet = MySQL.executeQuery(conn, command.toString());
			dataSet.tables.get(0).rows.genRowsMap();
			return dataSet.tables.get(0).rows.rowsMap;
		}
		...
		*/
		public long updateOverDueRepayment(Connection conn, long id,
				double lateFee,int overdueDay,String executeTime) throws SQLException {
			long returnId = -1;
			String command = "update t_repayment set isLate = 2,lateDay = "+overdueDay+",lateFI ="+lateFee+
			",executeTime = '"+executeTime+"'  where id ="+id;
		...
		*/
		 ...
		 * @Throws:
		*/
		public long deductedXLFee(Connection conn, long userId,
				double freeEducation) throws SQLException {
			StringBuffer command = new StringBuffer();
		...
		public Map<String, String> queryFirstVipById(Connection conn,Long userId) throws SQLException, DataException {
			StringBuffer command = new StringBuffer();
			command.append("select id from t_user where vipStatus =2 and feeStatus=1 and isFirstVip = 1 " +
					" and id ="+userId);
			DataSet dataSet = MySQL.executeQuery(conn, command.toString());
			return DataSetHelper.dataSetToMap(dataSet);
		}
		...
		public Map<String,String> queryFriendInfo(Connection conn,Long mId ,Long muid) throws SQLException, DataException{
			...
			DataSet dataSet = vlist.open(
							conn," username ",
							" mId=" + mId+" and muid ="+muid, "", 0, 1);
			return DataSetHelper.dataSetToMap(dataSet);
		}
		/**   
	...
	/*	public List<Map<String, Object>> queryOverDueInvestRepayment(
			...
			String command = "select a.id,a.recivedPrincipal,a.recivedInterest from t_invest_repayment a left join t_invest b "
					+ " on a.invest_id = b.id left join t_borrow c  on b.borrowId=c.id "
					+ " AND DATE_FORMAT(a.repayDate,'%Y-%m-%d') <= '"
					+ date
		...
		public List<Map<String, Object>> queryOverDueInvestRepayment(
				Connection conn, String date) throws SQLException, DataException {
			StringBuffer command = new StringBuffer();
			command.append("select a.id as id,a.repayid as repayId,b.islate as isLate,b.lateday as lateDay, c.borrowWay, c.borrowStatus,");
			command.append("a.recivedPrincipal as recivedPrincipal,a.recivedInterest as recivedInterest ,b.borrowId as borrowId  from " );
	...
	/**
			 ...
			 * @Throws:
			 */
			public long updateOverDueInvestRepayment(Connection conn, long id,long repayId,
				double lateFee,int lateDay,int isLate) throws SQLException {
			long returnId = -1;
