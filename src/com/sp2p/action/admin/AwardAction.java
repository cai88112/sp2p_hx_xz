...
public class AwardAction extends BasePageAction {
	...
	public String queryAwardLevel2Info() {
		String level2userName = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(request("level2userName"),
				null));
		String userName = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(request("userName"), null));
		String realName = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(request("realName"), null));
	...
	public String queryIoRInit() throws Exception {
		...
		String InvestorName = SqlInfusionHelper.filteSqlInfusion(userMap.get("username"));
		String t = SqlInfusionHelper.filteSqlInfusion(request("t"));
	...
	public String queryAwardLevel1Info() {
		...
		String level1userName = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(request("level1userName"),
				null));
		String level2userName = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(request("level2userName"),
				null));
	...
	public String queryAwardT() throws Exception {
		String level1userName = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(
	...
	public String queryLevel1level4Info() {
		...
		String level2userName = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(request("level2userName"),
				null));
		Integer level = Convert.strToInt(request("level"), -1);
		Integer dateStatus = Convert.strToInt(request("dateStatus"), -1);
		...
		if (dateStatus == 1) {// 有效期
			...
			startDateStr = SqlInfusionHelper.filteSqlInfusion(sdf.format(endDate));
		} else if (dateStatus == 2) {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			Date endDate = new Date();
			endDate.setYear(endDate.getYear() - 1);
			endDateStr = SqlInfusionHelper.filteSqlInfusion(sdf.format(endDate));
		}
		try {
	...
	public String addAwardDetailInit() throws SQLException, DataException {
		Long userId = request("userId") == null ? -100 : Convert.strToLong(
				request("userId"), -100);
		try {
	...
	public String queryAwardDetailByUserIdInit() {
		Long userId = request("userId") == null ? null : Convert.strToLong(
				request("userId"), -100);
	...
	public String queryLeve1AwardInfo() throws Exception {
		long level2userId = Convert.strToLong(request("level2userId"), -1);
		long level1userId = Convert.strToLong(request("level1userId"), -1);
		String username = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(request("luserName"), ""));
	...
	public String queryLeve2SumCount() throws Exception {
		String leve2Name = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(paramMap.get("level2userName"), ""));
		Admin admin = (Admin) session().getAttribute(IConstants.SESSION_ADMIN);
		long leve2userId = -1L;
