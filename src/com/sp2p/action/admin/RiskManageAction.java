...
public class RiskManageAction extends BasePageAction {
	...
	public String queryRiskList() {
		...
		String resource = paramMap.get("resource") == null ? "" : SqlInfusionHelper.filteSqlInfusion(paramMap
				.get("resource"));
		...
		String timeEnd = paramMap.get("timeEnd") == null ? "" : SqlInfusionHelper.filteSqlInfusion(paramMap
				.get("timeEnd"));
		String riskType = paramMap.get("riskType") == null ? "" : SqlInfusionHelper.filteSqlInfusion(paramMap
				.get("riskType"));
		try {
	...
	public String exportRiskList() {
		...
		try {
			String resource = Convert.strToStr(SqlInfusionHelper.filteSqlInfusion(request().getParameter(
					"resource")), "");
			resource = URLDecoder.decode(resource, "UTF-8");
			String timeStart = Convert.strToStr(SqlInfusionHelper.filteSqlInfusion(request().getParameter(
					"timeStart")), null);
			String timeEnd = Convert.strToStr(
					SqlInfusionHelper.filteSqlInfusion(request().getParameter("timeEnd")), null);
			String riskType = Convert.strToStr(SqlInfusionHelper.filteSqlInfusion(request().getParameter(
					"riskType")), "");
			...
			HSSFWorkbook wb = ExcelHelper.exportExcel("风险保障金记录", pageBean
