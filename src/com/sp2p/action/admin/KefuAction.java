...
public class KefuAction extends BasePageAction {
	...
	public String queryKefuForInfo() throws Exception {
		...
		String isF = paramMap.get("unfor") == null ? "-1" : paramMap
				.get("unfor")
				+ "";
		String isflag = paramMap.get("flag") + "";
		int flag = 3;
		if ("1".equals(isF)) {
			flag = 1;
		...
		List<Map<String, Object>> kefulist = kefulist = kefusService
				.querykefulist();
		request().setAttribute("kefulist", kefulist);
		request().setAttribute("kefuname", kefuname);
		request().setAttribute("flag", flag);
		kefusService.queryKefuForInfo(pageBean, SqlInfusionHelper
				.filteSqlInfusion(kefuname), SqlInfusionHelper
				.filteSqlInfusion(username), flag);
		return SUCCESS;
	}
	...
	public String addUserForkefubyId() throws Exception {
		...
		String kefuId = SqlInfusionHelper.filteSqlInfusion(paramMap.get("kid") + "");
		long result = -1;
	...
	public String addUserForkefu() throws Exception {
		String kefuid = SqlInfusionHelper.filteSqlInfusion(paramMap.get("selectid")
				+ "");
		String userid = SqlInfusionHelper.filteSqlInfusion(paramMap.get("userId")
				+ "");
		long result = -1;
		result = kefusService.updateUserKefu_(Convert.strToLong(kefuid, -1),
				Convert.strToLong(userid, -1));
		if (result > 0) {
			ServletHelper.returnStr(ServletActionContext.getResponse(), "1");
			return null;
		}
	...
	public String addKefu() throws Exception {
		String kefuName = SqlInfusionHelper.filteSqlInfusion(paramMap.get("name"));
		String imgPath = SqlInfusionHelper
				.filteSqlInfusion(paramMap.get("kefuImage"));
	...
	public String updateKefu() throws Exception {
		...
		try {
			...
			long result = kefusService.updateKefu(id, kefuName, imgPath, qq,
					remark);
			if (result > 0) {
				message = "更新成功";
			}
