...
public class ShoveApproveNoticeStyleAction extends BasePageAction {
	...
	public String queryNoticeTemplateAllInit() {
		int id = Convert.strToInt(request().getParameter("id"), 0);
		int notice_style = Convert.strToInt(request().getParameter(
				"notice_style"), -1);
	...
	public String updateNoticeTemplateInfo() throws Exception {
		String name = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(request("bean_name"), null));
		String template = SqlInfusionHelper.filteSqlInfusion(Convert.strToStr(request("bean_template"), null));
