...
public class HelpAction extends BasePageAction {
	...
	public String addHelp() throws Exception {
		...
		String publisher = Convert.strToStr(paramMap.get("publisher"), null); // Convert.strToLong(paramMap.get("adminId"),
																				// -1);
																				// //发布人
		...
		try {
			long result = callCenterService.addHelp(SqlInfusionHelper
					.filteSqlInfusion(title), helpId, serialCount, SqlInfusionHelper
					.filteSqlInfusion(content), SqlInfusionHelper
					.filteSqlInfusion(publisher), SqlInfusionHelper
					.filteSqlInfusion(dateTime));
			if (result > 0) {
				messageInfo = "添加成功";
				// 删除此分类下的分页数据
				...
				operationLogService.addOperationLog("t_help_question", admin
						.getUserName(), IConstants.INSERT, admin.getLastIP(),
						0, "添加" + IConstants.PRO_GLOBLE_NAME
								+ "IConstants.PRO_GLOBLE_NAME帮助问题数据", 2);
			}
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public String updateHelp() throws Exception {
		...
		String publishTime = Convert
				.strToStr(paramMap.get("publishTime"), null); // 发布时间
		...
		try {
			long result = callCenterService.updateHelp(SqlInfusionHelper
					.filteSqlInfusion(title), typeId, id, SqlInfusionHelper
					.filteSqlInfusion(publisher), SqlInfusionHelper
					.filteSqlInfusion(publishTime), serialCount);
			if (result > 0) {
				messageInfo = "修改成功";
				// 删除此分类下的分页数据,明细
				CacheManager.clearStartsWithAll(IConstants.CACHE_BZZX_PAGE_
				...
				operationLogService.addOperationLog("t_help_question", admin
						...
						0, "修改" + IConstants.PRO_GLOBLE_NAME + "帮助问题数据成功", 2);
	...
	public String updateHelpManager() throws Exception {
		...
		String messageInfo = "修改失败";
		try {
			long result = callCenterService.updateHelpManager(SqlInfusionHelper
					.filteSqlInfusion(title), SqlInfusionHelper
					.filteSqlInfusion(content), typeId, id, SqlInfusionHelper
					.filteSqlInfusion(publisher), SqlInfusionHelper
					.filteSqlInfusion(publishTime), serialCount);
			if (result > 0) {
				messageInfo = "修改成功";
	...
	public String queryHelpListPage() throws Exception {
		...
		try {
			callCenterService.queryHelpPage(pageBean, SqlInfusionHelper.filteSqlInfusion(title), helpId);
	...
	public String deleteHelp() throws Exception {
		String helpIds = request("commonId");
		String[] allIds = helpIds.split(",");// 进行全选删除的时候获得多个id值
		if (allIds.length > 0) {
			long tempId = 0;
			for (String str : allIds) {
				tempId = Convert.strToLong(SqlInfusionHelper.filteSqlInfusion(str), -1);
				if (tempId == -1) {
					return INPUT;
				}
			}
		...
		Map<String, String> map = callCenterService.getTypeId(Convert
				.strToLong(allIds[0], -1));
		if(map == null){
		...
		for (String id : allIds) {
			CacheManager.clearByKey(IConstants.CACHE_BZZX_INFO_ + SqlInfusionHelper.filteSqlInfusion(id));
		}
