...
public class ShoveBorrowAmountTypeAction extends BaseFrontAction {
	...
	public String updatgeBorrowAmount() {
		...
		String title = SqlInfusionHelper.filteSqlInfusion(request().getParameter("paramMap.title"));
		String descriptionm = SqlInfusionHelper.filteSqlInfusion(request().getParameter("paramMap.description"));
		int status = Convert.strToInt(
				request().getParameter("paramMap_status"), -1);
		String remark = SqlInfusionHelper.filteSqlInfusion(request().getParameter("paramMap.remark"));
		double init_credit = Convert.strToDouble(request().getParameter(
