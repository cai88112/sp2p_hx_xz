...
public class DebtAppAction extends BaseAppAction {
	...
	public String queryFrontAllDebt() throws IOException {
		...
		try {
			...
			assignmentDebtService.queryAllDebtApp(borrowTitle,debtSum, auctionBasePrice,
		...
		try {
			...
			if (map != null) {
				...
				if((paramMap.get("debtStatus")).equals("3")){
					paramMap.put("remainDays", DateHelper.remainDateToString(new Date(),DateUtils.addDays(new Date(),0)));
				}
...
//				request().setAttribute("userMap", userMap);
		...
		try {
			...
			if (debtMap != null && userMap != null) {
				if (debtMap.get("alienatorId").equals(userId + "")) {
					...
					JSONHelper.printObject(jsonMap); // 不能投自己转让的的债权
					return null;
				}
				...
				if (!pwd.equals(userMap.get("dealpwd").toLowerCase())) {
					...
					JSONHelper.printObject(jsonMap); // 交易密码不对
					return null;
				}
				...
				if (usableSum < (auctionPrice-oldAuctionPrice)) {
					...
					JSONHelper.printObject(jsonMap); // 可用余额不足
					return null;
				}
				...
				if (debtSum < auctionPrice) {
					...
					JSONHelper.printObject(jsonMap); // 大于债权金额
					return null;
				}
				...
				if (auctionBasePrice > auctionPrice) {
					...
					JSONHelper.printObject(jsonMap); // 小于最小竞拍金额
					return null;
				}
				...
				if (actionTimes >= 2) {
					...
					JSONHelper.printObject(jsonMap); // 该债权您只能竞拍两次
					return null;
				}
				...
				if(borrowerId==userId){
					...
					JSONHelper.printObject(jsonMap); // 借款者不能竞拍该债权
					return null;
				}
				...
				if(oldAuctionPrice >= auctionPrice){
					...
					JSONHelper.printObject(jsonMap); // 第二次竞拍金额应大于第一次竞拍金额
					return null;
				}
				...
				if(auctionHighPrice != -1 && auctionHighPrice > auctionPrice){
					...
					JSONHelper.printObject(jsonMap); // 竞拍金额要大于最高竞拍金额
					return null;
				}
				if(!"2".equals(debtMap.get("debtStatus"))){
					jsonMap.put("error", "11");
					jsonMap.put("msg", "竞拍失败");
					JSONHelper.printObject(jsonMap); //竞拍失败
					return null;
				}
				paramMap.put("userId", userId + "");
				paramMap.put("auctionTime", DateFormatUtils.format(new Date(), "yyyy-MM-dd HH:mm:ss"));
	...
	public String queryAcutionRecordInfo() throws IOException {
		...
		try {
			...
			String remainDays = DateHelper.remainDateToString(new Date(),				
			DateUtils.addDays(DateUtils.parseDate( debt.get("publishTime"), new String[]{"yyyy-MM-dd HH:mm:ss"} ),
