...
public class UserAppAction extends BaseAppAction {
	...
	public String login() throws IOException {
		...
		try {
			...
			String pwd = appInfoMap.get("pwd").toLowerCase();
			// 查找数据库对象中的enable属性
    ...
    log.info(randomCode);
			String content = "尊敬的客户您好,欢迎使用"+IConstants.PRO_GLOBLE_NAME+",手机验证码为:[" + randomCode + "]";
			// 获取短信接口url
...
//					.get("Password"), content, phone, "");
	...
	public String updateDealPwd() throws IOException {
		...
		try {
			...
			result = userService.updateDealPwd(userId, password.toLowerCase());
			if (result > 0) {
				jsonMap.put("error", "-1");
				jsonMap.put("msg", "修改成功");
				JSONHelper.printObject(jsonMap);
	...
	//用户提交图片审核
	public String addpastPicturdate(Long tmid,Long materAuthTypeId,User user,String imgPath) throws Exception{
		Map<String, String> jsonMap = new HashMap<String, String>();
		   //------add by houli
...
/*		if(materAuthTypeId==13){
			//视频
			if(5<allPicturecount){ServletHelper.returnStr(ServletActionContext.getResponse(), "13");return null;}//视频
		}*/
		     ...
		     if(result>0){
		    	 ...
		    	 try {
					 ...
					 if(newstatusmap!=null&&newstatusmap.size()>0){
							...
							user.setRealName(Convert.strToStr(newstatusmap.get("realName"),
									null));
							user.setKefuname(Convert.strToStr(newstatusmap.get("kefuname"),
									null));
							user.setUserName(Convert.strToStr(newstatusmap.get("username"),
									null));
