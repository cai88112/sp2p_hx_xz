...
//					AmountUtil au = new AmountUtil();
					...
					if (paymentMode == 1) {
						...
						// 先息后本
						earnMap = AmountHelper.earnCalculateSum(investAmount, borrowSum,
								annualRate, deadline, 0, isDayThe, 2);
						earnAmount = earnMap.get("msg") + "";
					} else if (paymentMode == 3) {
						// 秒还
						earnMap = AmountHelper.earnSecondsSum(investAmount, borrowSum,
	...
	public String financeInvest() throws Exception {
		...
		try {
			...
			Map<String, String> result = financeService.addBorrowInvest(idLong,
					user.getId(), "", amountDouble, getBasePath(), user
					.getUserName(), status, num, null,"");
