...
public class OnlinePaymentAction extends BaseFrontAction {
	...
	public String alipayReceive() throws Exception {
		...
		boolean verify_result = AlipayNotifyHelper.verify(params);// 验证参数是否是支付宝那边返回过来的。
		if (!verify_result) {
			createHelpMessage("支付失败！", "返回首页", "index.do");
		}
		...
		String paynumber = URLDecoder.decode(request("trade_no"), "utf-8");
		// 支付宝编号
...
//								userId);
						Long result = sendMessageService.noteSendUseXuanWu(content,
								userId);
