...
public class FrontUserAction extends BaseFrontAction {
	...
	public String updateUserHeadImg() throws Exception {
		// 获取用户的信息
		// User user = (User) session().getAttribute(IConstants.SESSION_USER);
		String userHeadImg =SqlInfusionHelper.filteSqlInfusion(request("userHeadImg"));
	...
	public String updateUserPassword() throws IOException {
		...
		try {
			// 获取用户的信息
			User user = (User) session().getAttribute(IConstants.SESSION_USER);
			String oldPassword =SqlInfusionHelper.filteSqlInfusion (paramMap.get("oldPassword"));
	 ...
	 * >=0){ ServletHelper.returnStr(ServletActionContext.getResponse(), "1"); }else { ServletHelper.returnStr(ServletActionContext.getResponse(), "2"); }
	 * 
	 * }else { ServletHelper.returnStr(ServletActionContext.getResponse(), "3"); } return null; }
	 */
	/**
	 * 用户修改地址初始化
	 * 
	 ...
	 * if(returnId >=0){ ServletHelper.returnStr(ServletActionContext.getResponse(), "1"); }else {
